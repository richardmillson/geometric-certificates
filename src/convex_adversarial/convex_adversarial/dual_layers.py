...
from .utils import Dense, full_bias
...
def select_layer(layer,
    ...
    if isinstance(layer, nn.Linear):
        ...
        if proj is not None and (norm_type == 'l1_median' or norm_type
                                 == 'l2_normal') and I.sum().item() > proj:
        ...
        print(layer)
        raise ValueError("No module for layer {}".format(
...
def conv2d(x, *args, **kwargs):
    ...
    while i < x.size(0):
        out.append(
            F.conv2d(x[i:min(i + batch_size, x.size(0))], *args, **kwargs))
        i += batch_size
...
def conv_transpose2d(x, *args, **kwargs):
    ...
    while i < x.size(0):
        out.append(
            F.conv_transpose2d(x[i:min(i + batch_size, x.size(0))], *args,
                               **kwargs))
        i += batch_size
    return torch.cat(out, 0)
...
class DualReLU(DualLayer):
    ...
    def bounds(self, network=None):
        ...
        zlI = self.zl[self.I]
        zl = (zlI * (-nu.t()).clamp(min=0)).mm(
            self.I_collapse).t().contiguous()
    ...
    def objective(self, *nus):
        ...
        zl = self.zl.view(n, -1)
        I = self.I.view(n, -1)
        return (nu.clamp(min=0) * zl.unsqueeze(1)).matmul(
            I.type_as(nu).unsqueeze(2)).squeeze(2)
...
class DualReLUProj(DualReLU):
    ...
    def __init__(self, zl, zu, k):
        ...
        if I.sum() > 0:
            nu[I.unsqueeze(1).expand_as(nu)] = nu.new(I.sum().item() *
                                                      k).cauchy_()
            nu_one[I] = 1
...
class DualDense(DualLayer):
        ...
        for i, W in enumerate(dense.Ws):
            ...
            if i < len(dense.Ws) - 1 and W is not None:
                ...
                net[idx].dual_ts = nn.ModuleList(
                    [dual_layer] + [None] *
                    (len(dense.Ws) - i - len(net[idx].dual_ts) - 1) +
                    list(net[idx].dual_ts))
    ...
    def forward(self, *xs):
        ...
        out = [
            W(*xs[:i + 1])
            for i, W in zip(range(-len(duals) + len(xs), len(xs)), duals)
            if W is not None
        ]
    ...
    def T(self, *xs):
        ...
        out = [
            W.T(*xs[:i + 1])
            for i, W in zip(range(-len(dual_ts) + len(xs), len(xs)), dual_ts)
            if W is not None
        ]
        # remove the non applicable outputs
        out = [o for o in out if o is not None]
    ...
    def bounds(self, network=None):
        fvals = list(
            W.bounds(network=network) for W in self.duals if W is not None)
        l, u = zip(*fvals)
        return sum(l), sum(u)
...
class DualBatchNorm2d(DualLayer):
    ...
    def __init__(self, layer, minibatch, out_features):
        ...
        self.ds = [
            ((bias - weight * mu / denom).unsqueeze(1).unsqueeze(2)).expand(
                out_features[1:]).contiguous()
        ]
