...
class PLNN(nn.Module):
    ...
    def make_adversarial_constraints(self, polytope, true_label, domain):
        ...
        for i in range(num_logits):
            if i == true_label:
                continue
            dec_bound = {'A': true_a - total_a[i], 'b': total_b[i] - true_b}
            new_facet = polytope.facet_constructor(None,
                                                   facet_type='decision',
    ...
    def compute_polytope_config(self,
                                configs,
                                comparison_form_flag=False,
        ...
        if (comparison_form_flag):
            polytope_A, polytope_b = utils.comparison_form(
                polytope_A, polytope_b)
    ...
    def compute_matrix(self, configs):
        ...
        for config, fc, layer_size in zip(configs, self.fcs, self.layer_sizes):
            nullifier = torch.Tensor(
                [config.numpy() for _ in range(0, layer_size)])
    ...
    def compute_interval_bounds(self,
                                domain_obj,
                                compute_logit_bounds=False,
                                as_tensor=False):
    ...
    def compute_improved_ia_bounds(self, domain_obj):
        ...
        def recurs(layer_num, lows, highs, weights, biases):
            assert len(lows) == len(highs) == len(weights) == len(
                biases) == layer_num
            # current layer
            low = lows[0]
            high = highs[0]
            ...
            # Recurse
            deeper_lows, deeper_highs = recurs(layer_num - 1, lows[1:],
                                               highs[1:],
                                               [w_prod] + weights[2:],
                                               [b_prod] + biases[2:])
        ...
        for layer_num, layer in enumerate(self.fcs[:-1]):
            new_lows, new_highs = recurs(layer_num + 1, running_lows,
                                         running_highs, running_weights,
                                         running_biases)
    ...
    def compute_dual_lp_bounds(self, domain_obj):
        ...
        dual_net = ca.DualNetwork(self.net,
                                  midpoint,
                                  domain_obj.linf_radius,
                                  box_bounds=box_bounds).dual_net
        bounds, dead_set = [], []
        for el in dual_net:
            if isinstance(el, ca.DualReLU):
                bounds.append(
                    torch.cat((el.zl.view(-1, 1), el.zu.view(-1, 1)), dim=1))
    ...
    def fast_lip(self, c_vector, l_q, on_off_neurons):
        ...
        layerwise_max = torch.where(low_bound > upp_bound, low_bound,
                                    upp_bound)
...
class LinearRegionCollection(object):
    ...
    def __init__(self,
        ...
        for config in return_obj.seen_polytopes:
            self.collection[config] = LinearRegion(plnn_obj,
                                                   config,
