...
import mister_ed.adversarial_attacks as attacks
...
class AdversarialAttackParameters(object):
    ...
    def attack(self, inputs, labels):
        ...
        selected_idxs = sorted(
            random.sample(list(range(num_elements)),
    ...
    def eval(self, ground_inputs, adv_inputs, labels, idxs, topk=1):
        ...
        return self.adv_attack_obj.eval(selected_grounds,
                                        adv_inputs,
                                        selected_labels,
                                        topk=topk)
    ...
    def eval_attack_only(self, adv_inputs, labels, topk=1):
        ...
        return self.adv_attack_obj.eval_attack_only(adv_inputs,
                                                    labels,
                                                    topk=topk)
    ...
    def switch_model(self, new_classifier, new_normalizer=None):
        ...
        """
        new_attack_obj = self.adv_attack_obj.switch_model(
            new_classifier, new_normalizer=new_normalizer)
        return AdversarialAttackParameters(
            new_attack_obj,
...
class TrainingProtocol(object):
    ...
    def __init__(self, minibatch_protocol, test_percentage):
        ...
        """
        assert minibatch_protocol in [
            'adv_only', 'adv_and_orig', 'duplicate_originals'
        ]
...
class AdversarialTraining(object):
    ...
    def set_verbosity_loglevel(self, level, verbosity_or_loglevel='verbosity'):
    ...
    def _attack_subroutine(self,
        ...
        for (key, param) in attack_parameters.items():
            ...
            needs_print = (self.verbosity_level >= 1
                           and minibatch_num % self.verbosity_adv
                           == self.verbosity_adv - 1)
            needs_log = (self.loglevel_level >= 1
                         and minibatch_num % self.loglevel_adv
                         == self.loglevel_adv - 1)
    ...
    def _cross_validate(self, test_loader, train_loss, attack_parameters,
                        epoch, regularize_adv_scale, regularize_adv_criterion):
        ...
        for i, data in enumerate(test_loader):
            ...
            mb_loss = self._minibatch_loss(inputs,
                                           labels,
                                           train_loss,
                                           attack_parameters,
                                           epoch,
    ...
    def train(self,
        ...
        if attack_parameters is not None:
            ...
            for param in attack_parameters.values():
                assert (
                    param.adv_attack_obj.classifier_net == self.classifier_net)
        ...
        for epoch in range(starting_epoch + 1, num_epochs + 1):
            ...
            if self.protocol.test_percentage > 0:
                train_loader, test_loader = utils.split_training_data(
                    data_loader, self.protocol.test_percentage)
            ...
            for i, data in enumerate(train_loader):
                ...
                loss = self._minibatch_loss(inputs, labels, train_loss,
                                            attack_parameters, epoch, i,
                                            adv_saver, regularize_adv_scale,
                ...
                if (verbosity_level >= 1 and i % verbosity_minibatch
                        == verbosity_minibatch - 1):
                    print('[%d, %5d] loss: %.6f' %
                          (epoch, i + 1,
                           running_loss_print / float(running_loss_print_mb)))
                ...
                if (loglevel_level >= 1
                        and i % loglevel_minibatch == loglevel_minibatch - 1):
    ...
    def train_from_checkpoint(self,
        ...
        self.classifier_net = checkpoints.load_state_dict(
            self.experiment_name, self.architecture_name, epoch,
            self.classifier_net)
        if self.use_gpu:
            self.classifier_net.cuda()
        ######################################################################
        ...
        best_test_loss = None
        if self.protocol.test_percentage > 0:
            _, test_loader = utils.split_training_data(
                data_loader, self.protocol.test_percentage)
            regularize_adv_criterion = None
            if regularize_adv_scale is not None:
                regularize_adv_criterion = self._get_regularize_adv_criterion()
            try:
                best_net = checkpoints.load_state_dict(self.experiment_name,
                                                       ...
                                                       'best',
                                                       self.classifier_net)
                best_net_computer = AdversarialTraining(
                    best_net, self.normalizer, self.experiment_name,
                    self.architecture_name)
                best_net_computer.set_verbosity_loglevel('low', 'verbosity')
                best_net_computer.set_verbosity_loglevel('low', 'loglevel')
                test_loss_best = best_net_computer._cross_validate(
                    test_loader, loss_fxn, attack_parameters, 0,
            ...
            if epoch != 'best':
                loaded_net = checkpoints.load_state_dict(
                    self.experiment_name, self.architecture_name, epoch,
                    self.classifier_net)
                loaded_net_computer = AdversarialTraining(
                    loaded_net, self.normalizer, self.experiment_name,
                    self.architecture_name)
