...
import mister_ed.custom_lpips.custom_dist_model as dm
    ...
    def switch_model(self, new_classifier, new_normalizer=None):
        ...
        if isinstance(self, (FGSM, PGD, SPSA)):
            new_loss = self.loss_fxn.switch_model(new_classifier,
                                                  new_normalizer)
            return self.__class__(new_classifier,
                                  new_normalizer,
                                  self.threat_model,
                                  new_loss,
...
class FGSM(AdversarialAttack):
    ...
    def __init__(self,
                 ...
                 loss_fxn,
                 manual_gpu=None):
        super(FGSM, self).__init__(classifier_net,
                                   normalizer,
                                   threat_model,
    ...
    def attack(self, examples, labels, step_size=0.05, verbose=True):
        ...
        # take gradients
        loss = self.loss_fxn.forward(perturbation(var_examples),
                                     var_labels,
...
class PGD(AdversarialAttack):
    ...
    def __init__(self,
                 ...
                 loss_fxn,
                 manual_gpu=None):
        super(PGD, self).__init__(classifier_net,
                                  normalizer,
                                  threat_model,
                                  manual_gpu=manual_gpu)
    ...
    def attack(self,
        ...
        if random_init is not False:
            ...
            self.validator(perturbation(var_examples),
                           var_labels,
                           iter_no="RANDOM")
...
class CarliniWagner(AdversarialAttack):
    ...
    def __init__(self,
        ...
        """
        super(CarliniWagner, self).__init__(classifier_net,
                                            normalizer,
                                            threat_model,
                                            manual_gpu=manual_gpu)

        assert (issubclass(distance_fxn, lf.ReferenceRegularizer)
                or distance_fxn == lf.PerturbationNormLoss)
        assert issubclass(carlini_loss, lf.CWLossF6)

        self.loss_classes = {
            'distance_fxn': distance_fxn,
    ...
    def _construct_loss_fxn(self, initial_lambda, confidence):
        ...
        losses = {
            'distance_fxn':
            self.loss_classes['distance_fxn'](None, use_gpu=self.use_gpu),
            'carlini_loss':
            self.loss_classes['carlini_loss'](self.classifier_net,
                                              self.normalizer,
                                              kappa=confidence)
        }
        scalars = {'distance_fxn': 1.0, 'carlini_loss': initial_lambda}
    ...
    def tweak_lambdas(cls, var_scale_lo, var_scale_hi, var_scale,
        ...
        scale_lo = utils.fold_mask(var_scale_lo.data, var_scale.data,
                                   successful_mask.data)
        scale = utils.fold_mask(downweights, upweights, successful_mask.data)
...
class SPSA(AdversarialAttack):
    ...
    def __init__(self,
                 ...
                 loss_fxn,
                 manual_gpu=None):
        super(SPSA, self).__init__(classifier_net,
                                   normalizer,
                                   threat_model,
                                   manual_gpu=manual_gpu)
        self.loss_fxn = loss_fxn
    ...
    def _gradient_estimate(self, perturbation, examples, labels,
        ...
        for spsa_batch in range(spsa_batch_size):
            ...
            for param in pert_params:
                ...
                param_rademacher = (param_rademacher * 2 -
                                    1.0) * spsa_pert_size
            ...
            low_loss = self.loss_fxn.forward(low_clone(examples),
                                             labels,
            ...
            high_loss = self.loss_fxn.forward(high_clone(examples),
                                              labels,
            ...
            if gradient_estimate is None:
                ...
                gradient_estimate = utils.add_tensor_list(
                    gradient_estimate, loss_diff)
    ...
    def attack(self,
        ...
        for iter_no in range(1, max_iterations + 1):
            ...
            gradient_estimate = self._gradient_estimate(
                perturbation, var_examples, var_labels, spsa_batch_size,
                spsa_pert_size)

            # Compute first/second moment estimates
            biased_m = utils.add_tensor_list(
                utils.scale_tensor_list(biased_m, adam_beta_1),
                utils.scale_tensor_list(gradient_estimate, (1 - adam_beta_1)))
            unbiased_m = utils.scale_tensor_list(
                biased_m, 1.0 / (1 - adam_beta_1**iter_no))
            ...
            biased_v = utils.add_tensor_list(
                ...
                utils.scale_tensor_list(
                    ...
                    (1 - adam_beta_2)))
            unbiased_v = utils.scale_tensor_list(
                biased_v, 1.0 / (1 - adam_beta_2**iter_no))
...
class JSMA(AdversarialAttack):
    ...
    def __init__(self,
        ...
        super(JSMA, self).__init__(classifier_net,
                                   normalizer,
                                   threat_model,
                                   manual_gpu=manual_gpu)
        self.loss_fxn = loss_fxn
    ...
    def _get_saliency_map(self, perturbation, examples, labels, targets):
        ...
        loss_val = self.loss_fxn.forward(perturbation(examples),
                                         labels,
                                         perturbation=perturbation)
    ...
    def attack(self,
        ...
        if targets is None:
            ...
            unmodified_targets = torch.randint_like(labels, 0,
                                                    self.num_classes - 1)
            targets = unmodified_targets + (unmodified_targets
                                            >= labels).long()
