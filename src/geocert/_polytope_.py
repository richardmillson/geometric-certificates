...
class Polytope(object):
    ...
    def from_polytope_dict(cls,
        ...
        linear_map = {
            'A': utils.as_numpy(polytope_dict['total_a']),
    ...
    def generate_facets_configs(self, seen_dict, missed_dict=None):
        ...
        ######################################################################
        upper_bound_proj = self.domain.minimal_facet_projections(
            self.ub_A, self.ub_b)
        new_potential_facets = []
        for idx in potential_facets:
    ...
    def _build_gurobi_model(self, indices_to_include=None):
        ...
        v_vars = [
            model.addVar(lb=lb_i(i), ub=ub_i(i), name='v%s' % i)
            for i in range(dim)
        ]
        ...
        _ = model.addConstrs(
            gb.LinExpr(a_rows[i], v_vars) <= b_rows[i] for i in range(m))
    ...
    def _is_dead(self, i):
        ...
        RETURNS:
            ...
            constraint. True o.w.
        """
        return (self.dead_constraints is not None and self.dead_constraints[i])

    def facet_constructor(self,
                          tight_idx,
                          facet_type='facet',
                          extra_tightness=None):

        return Face(self.ub_A,
                    self.ub_b, [tight_idx],
                    config=self.config,
                    domain=self.domain,
...
class Face(Polytope):
    ...
    def __init__(self,
                 ...
                 lipschitz_ub=None,
                 c_vector=None):
        super(Face, self).__init__(poly_a,
                                   poly_b,
                                   x_np,
                                   config=config,
                                   domain=domain,
    ...
    def linf_dist_gurobi(self, x):
        ...
        """
        v_vars = [
            v for v in self.gurobi_model.getVars() if v.VarName.startswith('v')
        ]
        ...
        try:
            self.gurobi_model.remove(
                self.gurobi_model.getConstrByName('facet'))
            self.gurobi_model.update()
        ...
        for lip_constr in self.gurobi_squire.lipschitz_constrs:
            try:
                self.gurobi_model.remove(
                    self.gurobi_model.getConstrByName('lipschitz'))
            except gb.GurobiError:
                pass
    ...
    def l2_dist(self, x):
        ...
        h = matrix(
            np.hstack([self.ub_b - self.ub_A.dot(x_row), constraint_d_b]))
        ...
        if quad_program_result[
                'status'] == 'optimal':  # or quad_program_result['status'] == 'unknown':
