...
def params_to_filename(experiment_name, architecture, epoch_val=None):
    ...
    if isinstance(epoch_val, int):
        ...
        return '.'.join([experiment_name, architecture, epoch_val, 'path'])

    glob_prefix = os.path.join(
        *[CHECKPOINT_DIR,
          '%s.%s.*' % (experiment_name, architecture)])
    re_prefix = '%s\.%s\.' % (experiment_name, architecture)
    re_suffix = r'\.path'

    valid_name = lambda f: bool(
        re.match(re_prefix + r'(\d{6}|best)' + re_suffix, f))

    safe_int_cast = lambda s: int(s) if s.isdigit() else s
    select_epoch = lambda f: safe_int_cast(
        re.sub(re_prefix, '', re.sub(re_suffix, '', f)))
    valid_epoch = lambda e: (
        (e == 'best') or (e >= (epoch_val or (0, 0))[0] and e <=
                          (epoch_val or (0, float('inf')))[1]))
...
def save_state_dict(experiment_name,
                    architecture,
                    epoch_val,
                    model,
                    k_highest=10):
    ...
    this_filename = params_to_filename(experiment_name, architecture,
                                       epoch_val)

    # Next clear up memory if too many state dicts
    current_filenames = [
        _ for _ in params_to_filename(experiment_name, architecture)
    ...
    torch.save(model.state_dict(),
               os.path.join(*[CHECKPOINT_DIR, this_filename]))
...
def load_state_dict_from_filename(filename, model):
    ...
    model.load_state_dict(
        right_dict(torch.load(os.path.join(*[CHECKPOINT_DIR, filename]))))
    return model
...
class CustomDataSaver(object):
    ...
    def save_minibatch(self, examples, labels):
        ...
        random_string = str(
            random.random())[2:]  # DO THIS BETTER WHEN I HAVE INTERNET
...
class CustomDataLoader(object):
    ...
    def __iter__(self):
        ...
        valid_random_names = set(
            os.path.basename(_).split('.')[0] for _ in files)
