...
from mister_ed.custom_lpips.base_model import BaseModel
...
class PNetLin(nn.Module):
    ...
    def forward(self, in0, in1):
        ...
        for i in range(len(self.lins)):
            val = val + torch.mean(
                torch.mean(self.lins[i].model(diffs[i]), dim=3), dim=2)
...
def normalize_tensor(in_feat, eps=1e-10):
    ...
    norm_factor = root_sum_square.view(og_size[0], 1, og_size[2],
                                       og_size[3]) + eps
...
class DistModel(BaseModel):
    ...
    def __init__(self, net='squeeze', manual_gpu=None):
        ...
        self.net = PNetLin(manual_gpu=manual_gpu,
                           pnet_tune=False,
                           use_dropout=True)
        weight_path = os.path.join(os.path.dirname(__file__), 'weights',
                                   '%s.pth' % net)
        self.net.load_state_dict(
            torch.load(weight_path, map_location=self.map_location))
