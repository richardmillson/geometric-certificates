    ...
    def recursive_midpoint(left, right):
        ...
        if midpoint_val < original_value:  # walk away from origin
            ...
            while new_val < original_value:
                ...
                new_guess = (midpoint[0], midpoint[1] + 2**k)
...
def equidistant_points(point_list, num_intervals):
    ...
    total_length = sum(
        l2_dist(point_list[i], point_list[i + 1])
    ...
    for i in range(num_intervals + 1):
        ...
        while not_satisfied:
            ...
            if segment_length - remainder <= dist_to_go:
                ...
                direction = tuple(
                    (endpoint[i] - startpoint[i]) / segment_length
                    for i in range(len(endpoint)))
                equipoint = tuple(
                    (dist_to_go + remainder) * direction[i] + startpoint[i]
