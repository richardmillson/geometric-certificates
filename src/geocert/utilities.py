...
def hamming_indices(str1, str2):
    ...
    indices = [
        index for index, (c1, c2) in enumerate(zip(str1, str2)) if c1 != c2
    ]
...
def cat_config(conf):
    ...
    """
    return torch.cat([_.cpu().type(torch.uint8).detach()
                      for _ in conf]).numpy()
...
def is_same_hyperplane(a1, b1, a2, b2, tolerance=global_tolerance):
    ...
    if fuzzy_equal(b1, 0, tolerance=tolerance):
        return (fuzzy_vector_equal(a1, a2, tolerance=tolerance)
                or fuzzy_vector_equal(a1, -a2, tolerance=tolerance))
...
def is_same_tight_constraint(a1, b1, a2, b2, tolerance=global_tolerance):
    ...
    return fuzzy_vector_equal(a1, a2, tolerance=tolerance) and fuzzy_equal(
        b1, b2, tolerance=tolerance)
...
def geometric_mean(M, x, t):
    ...
    if n == 1:
        ...
        t2 = M.variable()
        M.constraint(Var.hstack(t2, x.index(n - 1), t),
                     Domain.inPPowerCone(1 - 1.0 / n))
...
def det_rootn(M, t, n):
    ...
    M.constraint(Z.pick([[i, j] for i in range(n) for j in range(i + 1, n)]),
                 Domain.equalsTo(0.0))
    # DZ = Diag(Z)
    M.constraint(Expr.sub(DZ, Expr.mulElm(Z, Matrix.eye(n))),
                 Domain.equalsTo(0.0))
...
def MVIE_ellipse(A, b):
    ...
    try:
        with Model("lownerjohn_inner") as M:
            ...
            M.constraint(
                "qc", Expr.hstack(Expr.sub(b, Expr.mul(A, d)), Expr.mul(A, C)),
                Domain.inQCone())
...
def ray_shoot(x_0, d, ptope, facets, facet_bools=None):
    ...
    for index, facet_bool in enumerate(facet_bools):
        if facet_bool:
            ...
            face = facets[index]
            dist = (face.b_eq[0] - np.inner(face.a_eq[0], x_0)) / np.inner(
                face.a_eq[0], d)
            if dist >= 0:
...
def gurobi_LP(A_ub, b_ub, a_eq, b_eq, c, bounds=None, options=None):
    ...
    for i in range(0, m):
        opt_model.addConstraint(
            plp.LpConstraint(e=plp.lpDot(A_ub[i], x_vars),
    ...
    for i in range(0, m2):
        opt_model.addConstraint(
            plp.LpConstraint(e=plp.lpDot(a_eq[i], x_vars),
...
def MVIE_ellipse(A, b):
    ...
    with Model("lownerjohn_inner") as M:
        ...
        # (b-Ad, AC) generate cones
        M.constraint("qc",
                     Expr.hstack(Expr.sub(b, Expr.mul(A, d)), Expr.mul(A, C)),
                     Domain.inQCone())
...
def plot_polytopes_2d(poly_list,
                      colors=None,
                      alpha=1.0,
                      xylim=5,
                      ax=None,
    ...
    for poly, color in zip(poly_list, colors):
        ...
        if V is not None:
            ...
            new_ub_A = np.vstack((poly.ub_A, [[1, 0], [-1, 0], [0, 1], [0,
                                                                        -1]]))
            new_ub_b = np.hstack(
                (poly.ub_b, [xlim[1], -1 * xlim[0], ylim[1], -1 * ylim[0]]))
    ...
    for facet in facet_list:
        ...
        if vertices is not None and np.shape(vertices)[0] > 1:
            for vertex in vertices:
                equal = fuzzy_equal(np.dot(facet.a_eq[0], vertex),
                                    facet.b_eq[0])
                if equal:
                    facet_vertices.append(vertex)
            x1 = facet_vertices[0][0]
            x2 = facet_vertices[1][0]
            y1 = facet_vertices[0][1]
            y2 = facet_vertices[1][1]
            x = [x1, x2]
            y = [y1, y2]
            ...
            new_ub_A = np.vstack((facet.ub_A, [[1, 0], [-1, 0], [0, 1],
                                               [0, -1]]))
            new_ub_b = np.hstack(
                (facet.ub_b, [xlim[1], -1 * xlim[0], ylim[1], -1 * ylim[0]]))
            P2 = Polytope_2(new_ub_A, new_ub_b)
            V2 = ptope.extreme(P2)
            if V2 is not None:
                P2.plot(ax,
...
def plot_linf_norm(x_0, t, linewidth=1, edgecolor='black', ax=None):
    ...
    rect = patches.Rectangle((x_0[0] - t, x_0[1] - t),
                             2 * t,
                             2 * t,
                             linewidth=linewidth,
                             edgecolor=edgecolor,
...
def get_spaced_colors(n):
    ...
    return [(int(i[:2], 16) / 255.0, int(i[2:4], 16) / 255.0,
             int(i[4:], 16) / 255.0, 1) for i in colors]
...
class Polytope_2(ptope.Polytope):
    ...
    def __init__(self,
                 ...
                 vertices=None,
                 normalize=True):
        super(Polytope_2, self).__init__(A, b, minrep, chebR, chebX, fulldim,
                                         volume, vertices, normalize)
...
def binarize_relu_configs(relu_configs):
    ...
    """
    long_code = [
        element.data.numpy() for code in relu_configs for element in code
    ]
...
def plot_network_polytopes_sloppy(network, xylim, numpts, legend_flag=False):
    ...
    """
    _, unique_bin_acts, xs, num_activations = get_unique_relu_configs(
        network, xylim, numpts)
    color_dict = get_color_dictionary(unique_bin_acts)
    plt.figure(figsize=(10, 10))

    for unique_act in unique_bin_acts:
        indices = [
            index for index, element in enumerate(num_activations)
            if element == unique_act
        ]
        ...
        plt.scatter(x_pts_to_plot,
                    y_pts_to_plot,
                    label=str(unique_act),
                    c=colors)
...
def expand_xylim(alpha, xlim, ylim=None):
    if ylim is None:
        ylim = xlim
    expander = lambda lim, alpha: [
        elem - np.power(-1, i) * alpha * np.abs(lim[1] - lim[0])
        for i, elem in enumerate(lim)
    ]
