...
from geocert._polytope_ import Face, Polytope
...
class GeoCertReturn:
    ...
    def __init__(self,
                 ...
                 adv_ex=None,
                 seen_polytopes=None,
                 missed_polytopes=None,
                 polytope_graph=None,
                 lower_bound_times=None,
                 upper_bound_times=None,
                 status=None,
                 problem_type=None,
                 radius=None,
                 num_regions=None):
...
class GeoCert(object):
    ...
    bound_fxn_selector = {
        'ia': PLNN.compute_interval_bounds,
    ...
    def _compute_upper_bounds(self, x, true_label, extra_attack_kwargs=None):
        ...
        start = time.time()
        upper_bound, adv_ex = self._pgd_upper_bound(
            x, true_label, self.lp_norm, extra_kwargs=extra_attack_kwargs)
    ...
    def _pgd_upper_bound(self,
        ...
        linf_threat = ap.ThreatModel(ap.DeltaAddition, {
            'lp_style': 'inf',
            'lp_bound': 1.0
        })
        ...
        success_out = pert_out.collect_successful(
            self.net, normalizer, success_def='alter_top_logit')
    ...
    def run(self,
        ...
        if problem_type == 'min_dist':
            ...
            if compute_upper_bound is not False:
                ub_out = self._compute_upper_bounds(
                    x,
                    self.true_label,
                ...
                if adv_bound is not None:
                    upper_bound_times.append(
                        (time.time() - start_time, adv_bound))
        ...
        if potential == 'lipschitz':
            ...
            c_vector, lip_value = self.net.fast_lip_all_vals(
                x, dual_lp, self.on_off_neurons)
            self._verbose_print("LIPSCHITZ CONSTANTS", lip_value)
            self._verbose_print(c_vector[0].dot(self.net(x).squeeze()) /
                                lip_value[0])
        else:
            lip_value = None
            c_vector = None
        ...
        p_0 = Polytope.from_polytope_dict(
            p_0_dict,
            self.x_np,
            domain=self.domain,
        ...
        update_step_handle_polytope(self.net,
                                    self.x_np,
                                    self.true_label,
                                    sync_pq,
                                    seen_polytopes,
                                    self.domain,
                                    self.dead_constraints,
                                    p_0,
                                    self.lp_norm,
                                    pq_decision_bounds,
                                    potential,
        ...
        proc_args = (self.net, self.x_np, self.true_label, sync_pq,
                     ...
                     problem_type, polytope_graph, lower_bound_times,
                     start_time, upper_bound_times, max_runtime)
        ...
        if problem_type == 'min_dist':
            ...
            try:
                ...
                if problem_type == 'decision_problem':
                    ...
                    self._verbose_print("COUNTED %s LINEAR REGIONS" %
                                        len(seen_polytopes))
                    return_obj.status = 'SUCCESS'
...
def update_step_worker(piecewise_net,
                       x,
                       true_label,
                       pqueue,
                       seen_polytopes,
    ...
    while True:
        output = update_step_loop(piecewise_net,
                                  x,
                                  true_label,
                                  pqueue,
                                  seen_polytopes,
        ...
        if (max_runtime
                is not None) and (time.time() - start_time) > max_runtime:
            return output


def update_step_loop(piecewise_net,
                     x,
                     true_label,
                     pqueue,
                     seen_polytopes,
    ...
    poly_out = update_step_build_poly(piecewise_net,
                                      x,
                                      pqueue,
                                      seen_polytopes,
                                      heuristic_dict,
    ...
    return update_step_handle_polytope(piecewise_net,
                                       x,
                                       true_label,
                                       pqueue,
                                       seen_polytopes,
...
def update_step_build_poly(piecewise_net,
                           x,
                           pqueue,
                           seen_polytopes,
                           heuristic_dict,
...
def update_step_handle_polytope(piecewise_net,
                                x,
                                true_label,
                                pqueue,
                                seen_polytopes,
    ...
    ##########################################################################
    new_facets, rejects = new_poly.generate_facets_configs(
        seen_polytopes, missed_polytopes)

    if problem_type != 'count_regions':
        adv_constraints = piecewise_net.make_adversarial_constraints(
            new_poly, true_label, domain)
    else:
        adv_constraints = []
    ...
    for facet, (dist, proj) in outputs:
        ...
        if polytope_graph is not None:
            edge = (utils.flatten_config(new_poly.config), new_facet_conf)
...
def update_domain(new_domain,
    ...
    verbose_print('-' * 20,
                  "DOMAIN UPDATE | L_inf %.06f | L_2 %.06f" %
                  (linf_radius, l2_radius),
                  verbose=verbose)

    # Record the update in the upper_bound_times log
    attr = {'l_inf': 'linf_radius', 'l_2': 'l2_radius'}[lp_norm]

    upper_bound_times.append(
        (time.time() - start_time, getattr(new_domain, attr)))
    ...
    if potential == 'lipschitz':
        # Just assume binary classifiers for now
        dual_lp = utils.dual_norm(lp_norm)
        c_vector, lip_value = piecewise_net.fast_lip_all_vals(
            x, dual_lp, on_off_neurons)
        heuristic_dict['fast_lip'] = lip_value
