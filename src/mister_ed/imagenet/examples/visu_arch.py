...
if __name__ == '__main__':
    ...
    )  # https://stackoverflow.com/questions/19907442/python-explain-dict-attribute
    model.eval(
    )  # http://pytorch.org/docs/master/nn.html?highlight=eval#torch.nn.Module.eval
    print('')
    ...
    )  # http://pytorch.org/docs/master/_modules/torch/nn/modules/module.html#Module.state_dict
    ...
    for m in model.features:
        handles.append(
            m.register_forward_hook(print_info)
        )  # http://pytorch.org/docs/master/_modules/torch/nn/modules/module.html#Module.register_forward_pre_hook

    for m in model.classifier:
        handles.append(m.register_forward_hook(print_info))

    input = Variable(
        torch.randn(1, 3, 224, 224).float(), requires_grad=False
    )  # http://pytorch.org/tutorials/beginner/blitz/autograd_tutorial.html#sphx-glr-beginner-blitz-autograd-tutorial-py
    output = model(
        input
    )  # model(input) calls model.__call__(input) which calls model.forward(hook) and then calls the hooks
    ...
    train_data = Voc2007Classification(dir_datasets,
                                       'train')  # or val, test, trainval
    ...
    item = train_data[
        0]  # train_data contains a list of items (image, name, target)
    img_data = item[0]  # PIL.Image.Image
    img_name = item[1]  # string
    target = item[
        2]  #  torch.Tensor of size=20 (=nb_classes), contains 3 values: -1 (absence of class), 1 (presence of class), 0 (hard example)
    ...
    for class_id, has_class in enumerate(target):
        if has_class == 1:
            print('image {} has object of class {}'.format(
                img_name, train_data.classes[class_id]))

    ##########################################################################

    print('Load pretrained model on Imagenet')
    model.load_state_dict(
        model_zoo.load_url(model_urls[model_name], model_dir=dir_models))

    print('Display predictions')

    tf = transforms.Compose([
        transforms.Scale(
            224),  # rescale an RGB image to size 224^ (not a square)
        transforms.CenterCrop(
            224),  # extract a square of size 224 at the center of the image
    ...
    class_id = argmax[0]
    print('Image {} is of class "{}"'.format(img_name,
                                             imagenet_classes[class_id]))
    ...
    transforms.ToPILImage()(input_data[0]).save(
        path_img_input)  # save image using PIL
    ...
    def save_activation(self, input, output):
        ...
        for i in range(10):  #output.data.size(1)):
            path_img_output = os.path.join(
                dir_activations,
                'layer{}_{}_channel{}.png'.format(layer_id,
                                                  self.__class__.__name__, i))
            print('save output activation to ' + path_img_output)
            torchvision.utils.save_image(
                output.data.squeeze(0)[i], path_img_output
            )  # save image (of type Tensor) using torchvision
    ...
    for filter_id in range(weight.size(0)):
        path_param = os.path.join(
            dir_parameters, 'features.0.weight_filter{}.png'.format(filter_id))
        ...
        if 'features' in key and 'weight' in key:
            for filter_id in range(3):
                for channel_id in range(3):
                    path_param = os.path.join(
                        dir_parameters, '{}_filter{}_channel{}.png'.format(
                            key, filter_id, channel_id))
                    print('save ' + path_param)
                    torchvision.utils.save_image(
                        state_dict[key][filter_id][channel_id], path_param)
