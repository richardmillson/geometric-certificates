...
from mister_ed.utils import image_utils as img_utils
...
class AdversarialPerturbation(nn.Module):
    ...
    def __repr__(self):
        if isinstance(self.perturbation_params, tuple):
            output_str = "[Perturbation] %s: %s" % (
                self.__class__.__name__, self.perturbation_params[1])
            output_str += '\n['
    ...
    def collect_successful(self,
        ...
        correctly_class_idxs = set(
            utils.accuracy(adv_out, labels, return_correct_idxs=True)[1])
        success_idxs = torch.LongTensor([
            _ for _ in range(self.num_examples)
        ...
        return {
            'adversarials':
            self.adversarial_tensors().index_select(0, success_idxs),
            'originals':
            self.originals.index_select(0, success_idxs),
            'success_idxs':
            success_idxs
        }
...
class ThreatModel(object):
    ...
    def __repr__(self):
        return "[Threat] %s: %s" % (str(
            self.perturbation_class.__name__), self.param_kwargs)
    ...
    def perturbation_obj(self):
        return self.perturbation_class(self, self.param_kwargs,
                                       *self.other_args)
...
class DeltaAddition(AdversarialPerturbation):
    ...
    def random_init(self, param=True):
        ...
        self.delta = nn.Parameter(
            utils.random_from_lp_ball(self.delta.data, self.lp_style, bound))
    ...
    def clone_perturbation(self):
        ...
        self_mask = self.delta.new_ones((self.num_examples, ),
                                        dtype=torch.uint8)
    ...
    def scatter_perturbation(self, scatter_size, mask):
        ...
        scattered_params = utils.scatter_expand(self.delta, scatter_size, mask)
    ...
    def binsearch_closer(self, model, normalizer, labels, binsearch_steps=10):
        ...
        scale_hi = torch.ones(num_examples,
                              dtype=self.originals.dtype,
                              device=self.originals.device)
...
class ParameterizedXformAdv(AdversarialPerturbation):
    ...
    def random_init(self):
        ...
        param.data.add_(
            self.xform.identity_params(self.xform.img_shape) + random_perturb -
    ...
    def clone_perturbation(self):
        ...
        self_mask = list(self.xform.parameters())[0].new_ones(
            (self.num_examples, ), dtype=torch.uint8)
        return self.merge_perturbation(new_perturbation, self_mask)
    ...
    def scatter_perturbation(self, scatter_size, mask):
        ...
        identity_el = new_perturbation.xform.identity_params(
            1, *example_shape)[0]
...
class SequentialPerturbation(AdversarialPerturbation):
    ...
    def __init__(self,
        ...
        ARGS:
            ...
            preinit_pipelines: list[]
                if not None i
         """
        super(SequentialPerturbation,
              self).__init__(threat_model,
                             (perturbation_sequence, global_parameters))
    ...
    def clone_perturbation(self):
        ...
        new_perturbation = SequentialPerturbation(
            self.threat_model,
            layer_params,
    ...
    def merge_perturbation(self, other, self_mask):
        super(SequentialPerturbation,
              self).merge_perturbation(other, self_mask)

        new_pipeline = []
        for self_layer, other_layer in zip(self.pipeline, other.pipeline):
            new_pipeline.append(
                self_layer.merge_perturbation(other_layer, self_mask))
