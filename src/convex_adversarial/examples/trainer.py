...
def evaluate_robust(loader,
    ...
    print(' * Robust error {rerror.avg:.3f}\t'
          'Error {error.avg:.3f}'.format(rerror=robust_errors, error=errors))
...
def evaluate_madry(loader, model, epsilon, epoch, log, verbose):
    ...
    print(' * PGD error {perror.avg:.3f}\t'
          'Error {error.avg:.3f}'.format(error=errors, perror=perrors))
    return errors.avg
...
def robust_loss_cascade(models, epsilon, X, y, **kwargs):
    ...
    robust_ce, robust_err = rl(models[-1],
                               epsilon,
                               X,
                               y,
                               size_average=False,
...
def sampler_robust_cascade(loader, models, epsilon, batch_size, **kwargs):
    ...
    total = 0
    for i, (X, y) in enumerate(loader):
        print('Certifying minibatch {}/{} [current total: {}/{}]'.format(
            i, len(loader), total, len(dataset)),
              end='\r')
...
def evaluate_robust_cascade(loader, models, epsilon, epoch, log, verbose,
                            **kwargs):
    batch_time = AverageMeter()
    losses = AverageMeter()
    errors = AverageMeter()
    ...
    for i, (X, y) in enumerate(loader):
        ...
        if y.dim() == 2:
            y = y.squeeze(1)
        robust_ce, robust_err, ce, err, _ = robust_loss_cascade(
            models, epsilon, Variable(X), Variable(y), **kwargs)
        ...
        print(epoch, i, robust_ce.item(), robust_err, ce.item(), err, file=log)
        if verbose:
            endline = '\n' if i % verbose == 0 else '\r'
            # print(epoch, i, robust_ce.data[0], robust_err, ce.data[0], err)
            print('Test: [{0}/{1}]\t'
