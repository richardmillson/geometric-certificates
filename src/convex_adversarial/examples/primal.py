...
from convex_adversarial import Affine, DualNetBounds, full_bias
...
if __name__ == "__main__":
    ...
    for j, (X, y) in enumerate(loader):
        ...
        C = torch.eye(num_classes).type_as(X)[y].unsqueeze(1) - torch.eye(
            num_classes).type_as(X).unsqueeze(0)
        ...
        W = [
            l(Variable(torch.eye(
                l.in_features).cuda())).t().cpu().data.numpy() for l in affine
        ]
        ...
        for i0, x in enumerate(X.numpy()):
            ...
            for j0, c in enumerate(C[i0].numpy()):
                ...
                cons_upper = [(cp.mul_elemwise(
                    -(np.maximum(zu[i], 0) - np.maximum(zl[i], 0)), zhat[i]) +
                               cp.mul_elemwise((zu[i] - zl[i]), z[i + 1])
                               <= zu[i] * np.maximum(zl[i], 0) -
                               zl[i] * np.maximum(zu[i], 0))
                              for i in range(k - 1)]

                cons = cons_eq + cons_ball + cons_zero + cons_linear + cons_upper
                fobj = cp.Problem(cp.Minimize(c * zhat[-1]),
                                  cons).solve(verbose=False)
