...
def full_bias(l, n=None):
    ...
    if isinstance(l, nn.Linear):
        ...
        return sum(
            full_bias(layer, n=n) for layer in l.Ws if layer is not None)
...
def GR(epsilon):
    return (epsilon**2) / (
        -0.5 * np.log(1 + (2 / np.pi * np.log(1 + epsilon))**2) + 2 / np.pi *
        np.arctan(2 / np.pi * np.log(1 + epsilon)) * np.log(1 + epsilon))
...
def epsilon_from_model(model, X, k, delta, m):
    ...
    if l1_eps > 1:
        raise ValueError(
            'Delta too large / k too small to get probabilistic bound')
    return l1_eps
...
def get_epsilon(delta, k, alpha=1e-2):
    ...
    if epsilon > 1:
        raise ValueError(
            'Delta too large / k too small to get probabilistic bound (epsilon > 1)'
        )
