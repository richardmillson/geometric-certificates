...
import mister_ed.cifar10.wide_resnets as wide_resnets
...
def load_pretrained_cifar_resnet(flavor=32,
    ...
    correct_state_dict = {
        re.sub(r'^module\.', '', k): v
        for k, v in bad_state_dict['state_dict'].items()
    }
...
def load_cifar_data(train_or_val,
                    extra_args=None,
                    dataset_dir=None,
                    normalize=False,
                    batch_size=None,
    ...
    if normalize:
        normalizer = transforms.Normalize(mean=CIFAR10_MEANS, std=CIFAR10_STDS)
