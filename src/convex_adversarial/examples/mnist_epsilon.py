...
if __name__ == "__main__":
    ...
    for j, (X, y) in enumerate(loader):
        ...
        epsilon = Variable(args.epsilon * torch.ones(X.size(0)).cuda(),
                           requires_grad=True)
        ...
        c = Variable(
            torch.eye(num_classes).type_as(X.data)[out.data].unsqueeze(1) -
            torch.eye(num_classes).type_as(X.data).unsqueeze(0))
        I = (~(out.data.unsqueeze(1) == torch.arange(num_classes).type_as(
            out.data).unsqueeze(0)).unsqueeze(2))
        ...
        for i in range(args.niters):
            ...
            alpha = args.alpha
            epsilon0 = Variable(
                (epsilon - alpha * (f_max / (epsilon.grad))).data,
                requires_grad=True)
        ...
        del X, y
    print(l)
    torch.save(torch.cat(correct, 0),
               '{}_correct_epsilons.pth'.format(args.prefix))
    torch.save(torch.cat(incorrect, 0),
               '{}_incorrect_epsilons.pth'.format(args.prefix))
